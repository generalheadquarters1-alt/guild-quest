...
2. SQL Editorでマイグレーションを順番に実行します。
   ...
   - `supabase/migrations/011_adventurer_task_original_owner.sql`
   - `supabase/migrations/012_rpg_growth_and_expedition_result.sql`
...
遠征は、クエスト達成後も定期的にアプリを開きたくなる放置RPG要素です。冒険者を鍛えるほど成功率が上がり、成功・失敗に応じて報酬と育成結果が変わります。

基本ループ:

1. 任務を完了、またはクエストを討伐完了します。
2. 遠征チケットを獲得します。
3. 「冒険者育成」で訓練・休息・ギルド集会・装備整備を行います。
4. PC版は左メニューの「遠征」、スマホ版は「自分」タブ内の遠征セクションを開きます。
5. 成功率と必要信頼度を確認し、遠征先を選んで「出発」します。
6. 所要時間が経過したら「報酬を受け取る」で成功/失敗判定が行われ、EXP、GOLD、素材、ギルドEXPを獲得します。
...
遠征成功率は、遠征先の基礎成功率に冒険者の状態を加味して計算します。

主な補正:

- Lv: 高いほど成功率が上がります。
- 士気: 高いほど成功率が上がり、低いと下がります。
- 疲労: 高いほど成功率が下がります。90以上では出発できません。
- 熟練度: 高いほど成功率が上がります。
- 信頼度: 高難度遠征の解放条件になります。
- 装備: 装備ごとの成功率補正があります。
- 装備耐久: 低いと成功率が下がり、5以下では出発できません。
- 職業: 遠征先や報酬に補正が入ります。

成功率は10%〜98%に収まります。失敗しても完全な無報酬にはならず、少量のEXPとGOLDを獲得できます。

### 冒険者育成ステータス

| ステータス | 意味 |
| --- | --- |
| 士気 | 遠征成功率に影響します。高いほど有利です。 |
| 疲労 | 遠征成功率を下げます。高すぎると遠征に出せません。 |
| 熟練度 | 遠征成功率と成長感に関わります。訓練で上げます。 |
| 信頼度 | 高難度遠征の参加条件です。ギルド集会や遠征成功で上がります。 |
| 装備 | 成功率補正を持ちます。将来的な装備作成に拡張予定です。 |
| 装備耐久 | 遠征出発で減少します。低いと成功率が下がります。 |
| 職業 | 遠征先やGOLD獲得量などの補正に使います。 |

育成アクション:

- 訓練する: 熟練度が上がり、疲労が増え、士気が少し下がります。
- 酒場で休息: 疲労を回復し、士気を上げます。
- ギルド集会: 信頼度と士気を上げます。
- 装備整備: 装備耐久を回復し、疲労が少し増えます。

アプリ起動時や遠征画面表示時には、時間経過に応じて士気・疲労・装備耐久がゆるやかに変化します。将来的には、遠征素材を使った装備作成、職業変更、ギルド施設強化へ拡張する想定です。

遠征機能を使うには `supabase/migrations/006_expeditions.sql` と `supabase/migrations/012_rpg_growth_and_expedition_result.sql` を実行してください。
